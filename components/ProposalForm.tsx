...
const brandColors = [
    { name: 'Indigo', hex: '#4f46e5', className: 'bg-indigo-600' },
    { name: 'Slate', hex: '#475569', className: 'bg-slate-600' },
    { name: 'Green', hex: '#16a34a', className: 'bg-green-600' },
    { name: 'Rose', hex: '#e11d48', className: 'bg-rose-600' },
    ...
    return (
        <form onSubmit={onSubmit} className="bg-white p-6 sm:p-8 rounded-xl shadow-lg border border-slate-100 space-y-6">
            ...
            <div className="pt-4 border-t border-slate-200">
                ...
                <p className="text-sm text-slate-500 mt-1">Select a primary color for proposal headings.</p>
                <div className="flex space-x-3 mt-3">
                    {brandColors.map((color) => (
                        <button
                            ...
                            className={`w-8 h-8 rounded-full ${color.className} transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 ${input.brandColor === color.hex ? 'ring-indigo-500' : 'ring-transparent'}`}
                            aria-label={`Select ${color.name} color`}
                            aria-pressed={input.brandColor === color.hex}
                        >
                            {input.brandColor === color.hex && (
